...
def save_results(sol_a, sol_b, t_points, proximity, ln_deltas, lamb, eps):
    ...
    with (
        ...
        for i in tqdm(range(1, len(t_points)), desc="Write file"):
            ...
            row_a = {
                ...
                "lambda": lamb[i - 1],
                "proximity": -np.log10(proximity[i - 1]),
                "x_1": sol_a[0, i],
                "y_1": sol_a[1, i],
                "z_1": sol_a[2, i],
                "x_2": sol_a[3, i],
...
def solve_eq(cond_a, cond_b, n_steps, dt, t_start, masses):
    ...
    for i in range(1, n_steps):
        ...
        prox = np.linalg.norm(sa - cond_a)
        proximity_list[i - 1] = prox
...
def main():
    ...
    masses_arr = np.array(cfg.masses, dtype=np.float64)
    sol_a, sol_b, t_points, ln_deltas, lambdas, proximity = solve_eq(
        cond_a, cond_b, cfg.n_steps, cfg.dt, cfg.t_start, masses_arr
    )
    generate_proximity_heatmap(cfg)
    # --- File Output ---
    save_results(sol_a, sol_b, t_points, proximity, ln_deltas, lambdas, cfg.eps)
